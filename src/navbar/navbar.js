...
const Navbar = {
  size(el) {
    ...
    if ($el.hasClass('navbars')) {
      $el = $el.children('.navbar').forEach((navbarEl) => {
  ...
  hide(el, animate = true, hideStatusbar = false, hideOnlyCurrent = false) {
    ...
    if (isDynamic) {
      $el.children('.navbar').forEach((subEl) => {
  ...
  initNavbarOnScroll(pageEl, navbarEl, needHide, needCollapse, needTransparent) {
    ...
    if (needCollapse || (needHide && isLarge)) {
      ...
      if (navbarTitleLargeHeight && navbarTitleLargeHeight.indexOf('px') >= 0) {
        ...
        if (Number.isNaN(navbarTitleLargeHeight) && $titleLargeEl.length) {
          ...
          else if (app.theme === 'pc') navbarTitleLargeHeight = 38;
        }
    ...
    function calcScrollableDistance() {
      $pageEl.find('.page-content').forEach((pageContentEl) => {
...
export default {
  ...
  on: {
    ...
    'panelOpen panelSwipeOpen modalOpen': function onPanelModalOpen(instance) {
      const app = this;
      instance.$el.find('.navbar:not(.navbar-previous)').forEach((navbarEl) => {
        app.navbar.size(navbarEl);
      });
    },
    tabShow(tabEl) {
