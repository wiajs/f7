...
export default {
  ...
  create() {
    ...
    if (typeof app.params.input.scrollIntoViewOnFocus === 'undefined') {
      app.params.input.scrollIntoViewOnFocus = device.android;
    }
  ...
  on: {
    ...
    tabMounted(tabEl) {
      const app = this;
      const $tabEl = $(tabEl);
      $tabEl.find('.item-input, .input').forEach((itemInputEl) => {
        const $itemInputEl = $(itemInputEl);
        $itemInputEl.find('input, select, textarea, [contenteditable]').forEach((inputEl) => {
          const $inputEl = $(inputEl);
          if (Input.ignoreTypes.indexOf($inputEl.attr('type')) >= 0) return;
          app.input.checkEmptyState($inputEl);
        });
      });
      $tabEl.find('textarea.resizable').forEach((textareaEl) => {
    ...
    pageInit(page) {
      ...
      $pageEl.find('.item-input, .input').forEach((itemInputEl) => {
        const $itemInputEl = $(itemInputEl);
        $itemInputEl.find('input, select, textarea, [contenteditable]').forEach((inputEl) => {
          const $inputEl = $(inputEl);
          if (Input.ignoreTypes.indexOf($inputEl.attr('type')) >= 0) return;
      ...
      $pageEl.find('textarea.resizable').forEach((textareaEl) => {
        app.input.resizeTextarea(textareaEl);
      });
    },
    'panelBreakpoint panelCollapsedBreakpoint panelResize panelOpen panelSwipeOpen resize viewMasterDetailBreakpoint':
      ...
      if (instance && instance.$el) {
          instance.$el.find('textarea.resizable').forEach((textareaEl) => {
          app.input.resizeTextarea(textareaEl);
        });
      } else {
          $('textarea.resizable').forEach((textareaEl) => {
          app.input.resizeTextarea(textareaEl);
        });
      }
    },
