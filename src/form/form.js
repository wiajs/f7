...
const { extend, serializeObject } = Utils;
...
function formToData(formEl) {
  ...
  const skipNames = [];
  $formEl.find('input, select, textarea').forEach((inputEl) => {
    ...
    if (tag === 'select' && $inputEl.prop('multiple')) {
      skipNames.push(name);
      data[name] = [];
      $formEl.find(`select[name="${name}"] option`).forEach((el) => {
      ...
      switch (type) {
        case 'checkbox':
          skipNames.push(name);
          data[name] = [];
          $formEl.find(`input[name="${name}"]`).forEach((el) => {
...
export default {
  ...
  on: {
    ...
    pageBeforeRemove(page) {
      const app = this;
      page.$el.find('.form-store-data').forEach((formEl) => {
        app.form.storage.destroy(formEl);
      });
    },
    pageInit(page) {
